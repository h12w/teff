...
Extensions
----------
Core definitions only include value and list. Format for richer data structures
can be mapped onto the two primitives. A TFF implementation should provide an
implementation specific way (i.e. a set of interfaces) for users to define the
format of their own types.
...
To represent an array of list, the anonymous symbol "_" is introduced. e.g.
...
again as a parent-child relation.

    map_key         ::= value
    map_value       ::= node
    key_value       ::= map_key (indent map_value unindent)?
    map             ::= <list of key_value>
